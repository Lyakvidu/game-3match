...
const IS_LOCAL_TEST = ["localhost", "127.0.0.1", "::1"].includes(window.location.hostname);
const DEBUG_ADS = IS_LOCAL_TEST && QUERY.get("debugAds") === "1";
...
const LOCAL_PROGRESS_KEY = VK_USER_ID ? `${PROGRESS_KEY}_vk_${VK_USER_ID}` : PROGRESS_KEY;
...
const VK = {
  ...
  async init() {
    ...
    try {
      ...
      try {
        await withTimeout(this.bridge.send("VKWebAppSetViewSettings", {
      ...
      // Нативный баннер платформы размещается снизу и уменьшает viewport,
      // поэтому не перекрывает игровые кнопки и соответствует правилам VK.
  ...
  async showRewarded() {
    ...
    try {
      const check = await withTimeout(this.bridge.send("VKWebAppCheckNativeAds", { ad_format: "reward" }), 3000);
      if (!check.result) return false;
      const result = await withTimeout(this.bridge.send("VKWebAppShowNativeAds", { ad_format: "reward" }), 120000);
  ...
  async showInterstitial() {
    if (!this.ready) return false;
    try {
      const check = await withTimeout(this.bridge.send("VKWebAppCheckNativeAds", { ad_format: "interstitial" }), 3000);
      if (!check.result) return false;
      const result = await withTimeout(this.bridge.send("VKWebAppShowNativeAds", { ad_format: "interstitial" }), 120000);
      return result.result === true;
    } catch (_) {
      return false;
    }
  ...
  async showBanner() {
    if (!this.ready) return false;
    try {
      const check = await withTimeout(this.bridge.send("VKWebAppCheckBannerAd", {}), 2500);
      if (!check.result) return false;
      const result = await withTimeout(this.bridge.send("VKWebAppShowBannerAd", {
      ...
      // Недоступный баннер не должен мешать запуску и игровому процессу.
      return false;
    }
  },

  getPublicAppUrl() {
    const rawAppId = this.launchParams && this.launchParams.vk_app_id
      ? this.launchParams.vk_app_id
      : QUERY.get("vk_app_id");
    const appId = String(rawAppId || "").trim();
    return /^\d+$/.test(appId) ? `https://vk.com/app${appId}` : null;
  },
  ...
  async share() {
    ...
    if (this.ready) {
      try {
        await withTimeout(this.bridge.send("VKWebAppShare", { link: publicAppUrl }), 15000);
        return true;
      } catch (_) {
        return false;
      }
...
function parseProgress(raw) {
  ...
  try {
    ...
    // Разблокировка всегда следует за реально пройденными уровнями.
    // Поэтому новый или повреждённый профиль не может случайно начать с 3-го уровня.
    let highestSequentialWin = 0;
    for (const gameLevel of LEVELS) {
      if ((clean.stars[gameLevel.id] || 0) <= 0) break;
      highestSequentialWin = gameLevel.id;
    }
    clean.unlocked = Math.min(clean.unlocked, Math.max(1, highestSequentialWin + 1));
...
function getLocalProgress() {
  try {
    return parseProgress(localStorage.getItem(LOCAL_PROGRESS_KEY))
      || (!IS_VK_LAUNCH ? parseProgress(localStorage.getItem(LEGACY_PROGRESS_KEY)) : null);
  } catch (_) {
    return null;
  }
}
...
async function syncProgressWithVK() {
  ...
  if (vkProgress && vkProgress.updatedAt > progress.updatedAt) {
    progress = vkProgress;
    try { localStorage.setItem(LOCAL_PROGRESS_KEY, JSON.stringify(progress)); } catch (_) {}
    syncSettingsUi();
    updateMenuStats();
...
function saveProgress() {
  ...
  try {
    localStorage.setItem(LOCAL_PROGRESS_KEY, raw);
  } catch (_) {
...
musicEl.volume = 0.16;
const SFX_MASTER = 1.9;
let audioContext = null;
let audioUnlocked = false;
let noiseBuffer = null;
let cascadeSoundVariant = 0;

function unlockAudio() {
  const needsWarmup = !audioUnlocked || !audioContext || audioContext.state !== "running";
  audioUnlocked = true;

  // Safari и VK WebView разрешают Web Audio только непосредственно внутри
  // пользовательского жеста. Создаём, возобновляем и «прогреваем» контекст
  // прямо на первом касании, а не когда комбинация завершится после анимации.
  try {
    ...
    // Следующее касание повторит попытку разблокировки.
  }
...
function playTone(
  ...
  try {
    ...
    const peakVolume = Math.min(0.14, volume * SFX_MASTER);
...
function playShatterSound(strength = 1) {
  if (!progress.settings.sfx || !audioUnlocked) return;
  try {
    const AudioContextClass = window.AudioContext || window.webkitAudioContext;
    if (!AudioContextClass) return;
    ...
    gain.gain.setValueAtTime(Math.min(0.13, 0.052 * SFX_MASTER * strength), now);
    ...
    playTone(520 + Math.random() * 180, 0.07, 0.026 * strength);
  } catch (_) {}
}

function playBombSound() {
  playShatterSound(1.35);
  playTone(155, 0.24, 0.065, 0);
  playTone(92, 0.3, 0.052, 0.035);
  playTone(780, 0.08, 0.035, 0.02);
}

function playCascadeSound(chainLength, matchedCount) {
  if (!progress.settings.sfx || !audioUnlocked) return;
  if (chainLength === 1 && matchedCount < 4) return;

  const tier = chainLength >= 2
    ? Math.min(chainLength, 6)
    : (matchedCount >= 5 ? 3 : 2);
  const patterns = {
    2: [392, 523, 659],
    3: [440, 554, 659, 880],
    4: [523, 659, 784, 1046],
    5: [587, 740, 880, 1175, 1480],
    6: [659, 784, 988, 1318, 1568, 1976],
  };
  const transposeOptions = [0.944, 1, 1.059];
  const transpose = transposeOptions[(cascadeSoundVariant++ + matchedCount + tier) % transposeOptions.length];
  const notes = patterns[tier];
  const spacing = tier >= 5 ? 0.055 : 0.07;
  const waveform = cascadeSoundVariant % 2 === 0 ? "triangle" : "sine";

  // Низкий удар подчёркивает падение нового каскада, а восходящая фраза
  // меняет высоту и тембр, чтобы серии не звучали одинаково.
  playTone(142 + tier * 9, 0.2, 0.026 + tier * 0.002, 0, "sine", 82 + tier * 4);
  notes.forEach((frequency, index) => {
    const isLast = index === notes.length - 1;
    playTone(
      frequency * transpose,
      isLast ? 0.17 : 0.09,
      (isLast ? 0.03 : 0.021) + tier * 0.0015,
      0.025 + index * spacing,
      isLast && tier >= 4 ? "sawtooth" : waveform,
      isLast && tier >= 4 ? frequency * transpose * 1.08 : null,
    );
  });

  if (tier >= 4) {
    const finaleDelay = 0.04 + notes.length * spacing;
    playTone(notes[notes.length - 1] * transpose * 2, 0.19, 0.015 + tier * 0.001, finaleDelay, "sine");
  }
}
...
// Не используем once: после сворачивания приложения WebView может снова
// приостановить аудио, и ближайшее касание должно мгновенно его разбудить.
...
async function resolveCascade() {
  ...
  while (true) {
    ...
    showComboBanner(cascadeIndex + 1, matches.size);
    playShatterSound(1 + Math.min(cascadeIndex, 4) * 0.13);
    playCascadeSound(cascadeIndex + 1, matches.size);
    ...
    for (const key of matches) {
      ...
      const gem = board[row][col];
      if (!gem) continue;
      spawnGemBurst(gem, matches.size > 10 ? 3 : 5, 0.9 + cascadeIndex * 0.08);
...
function spawnParticles(row, col, type, count = 5, power = 1) {
  const centerX = col * cellSize + cellSize / 2;
  const centerY = row * cellSize + cellSize / 2;
  const color = GEM_PARTICLE_COLORS[type] || "#ffffff";

  for (let index = 0; index < count; index++) {
    const particle = document.createElement("i");
    const angle = (Math.PI * 2 * index) / count + (Math.random() - 0.5) * 0.55;
    const distance = cellSize * power * (0.48 + Math.random() * 0.58);
    const duration = 430 + Math.random() * 190;
    particle.className = `burst-particle${index % 3 === 0 ? " spark" : ""}`;
    particle.style.left = `${centerX}px`;
    particle.style.top = `${centerY}px`;
    particle.style.setProperty("--particle-x", `${Math.cos(angle) * distance}px`);
    particle.style.setProperty("--particle-y", `${Math.sin(angle) * distance}px`);
    particle.style.setProperty("--particle-rotate", `${120 + Math.random() * 260}deg`);
    particle.style.setProperty("--particle-size", `${Math.max(4, cellSize * (0.08 + Math.random() * 0.07))}px`);
...
function spawnGemBurst(gem, count = 5, power = 1) {
  spawnParticles(gem.row, gem.col, gem.type, count, power);
}

function spawnBombEffect(row, col) {
  const centerX = col * cellSize + cellSize / 2;
  const centerY = row * cellSize + cellSize / 2;
  ...
  ring.style.setProperty("--ring-size", `${cellSize * 1.25}px`);
  ...
  flash.style.setProperty("--flash-size", `${cellSize * 1.75}px`);
  boardEl.appendChild(flash);

  for (let index = 0; index < 18; index++) {
    const randomTypeName = GEM_TYPES[rnd(level.colors)];
    spawnParticles(row, col, randomTypeName, 1, 1.45 + Math.random() * 0.55);
  }
...
function showComboBanner(chainLength, matchedCount) {
  let title = "";
  let subtitle = "";

  if (chainLength >= 6) {
    title = "БОЖЕСТВЕННО!";
    subtitle = `Каскад ×${chainLength}`;
  } else if (chainLength === 5) {
    title = "ЛЕГЕНДАРНАЯ ЦЕПЬ!";
    subtitle = "Кристаллы не остановить";
  } else if (chainLength === 4) {
    title = "КРИСТАЛЬНЫЙ ШТОРМ!";
    subtitle = "Четверной каскад";
  } else if (chainLength === 3) {
    title = "ТРОЙНОЙ РАЗНОС!";
    subtitle = "Каскад ×3";
    ...
    title = "МЕГА-СВЯЗКА!";
    subtitle = `${matchedCount} кристаллов одним ударом`;
  } else if (matchedCount === 4) {
    title = "СОЧНАЯ ЧЕТВЁРКА!";
    subtitle = "Отличный ход";
  } else {
    return;
  }
  ...
  document.getElementById("combo-subtitle").textContent = subtitle;
  banner.classList.remove("active", "epic");
  banner.dataset.tier = String(Math.min(6, chainLength >= 2 ? chainLength : (matchedCount >= 5 ? 3 : 2)));
  void banner.offsetWidth;
  banner.classList.toggle("epic", chainLength >= 4 || matchedCount >= 5);
  banner.classList.add("active");
  window.clearTimeout(comboTimer);
  comboTimer = window.setTimeout(() => banner.classList.remove("active", "epic"), 1200);
}
...
async function showHint() {
  ...
  if (hintCredits <= 0) {
    const granted = await unlockHintPack();
    if (!granted) return;
  }
  ...
  playTone(680, 0.1, 0.032);
  await sleep(1000);
  pair.forEach((gem) => gem.el && gem.el.classList.remove("hint"));
}

async function unlockHintPack() {
  const button = document.getElementById("booster-hint");
  button.disabled = true;
  locked = true;
  showToast("Посмотри рекламу и получи 3 подсказки");
  const rewarded = await VK.showRewarded();
  locked = false;
  button.disabled = false;

  if (!rewarded) {
    showToast(VK.ready ? "Реклама сейчас недоступна" : "Дополнительные подсказки доступны внутри VK");
    return false;
  }

  hintCredits = 3;
  updateHintUi();
  playTone(820, 0.13, 0.04);
  showToast("Начислено 3 дополнительные подсказки");
  return true;
}
...
function updateHintUi() {
  ...
  const needsAd = hintCredits <= 0;
  label.textContent = needsAd ? "За рекламу" : `Подсказка · ${hintCredits}`;
  icon.textContent = needsAd ? "▶" : "💡";
  button.classList.toggle("ad-required", needsAd);
  button.setAttribute("aria-label", needsAd ? "Подсказка за рекламу" : `Подсказка, бесплатно осталось: ${hintCredits}`);
}
...
async function useBombOn(gem) {
  ...
  spawnBombEffect(centerRow, centerCol);
...
async function startLevel(id) {
  ...
  clearSelection();
  document.getElementById("booster-bomb").classList.remove("active-armed");
  document.getElementById("combo-banner").classList.remove("active", "epic");
